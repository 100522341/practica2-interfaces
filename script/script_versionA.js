...
function activarBoton() { //Funcion que activa el boton guardar si esta marcada la casilla politica de privacidad
  boton.disabled = !politica.checked;
}
politica.addEventListener('change', activarBoton);//Cada vez que el usuario marque o desmarque se ejcuta la funcion activar boton
activarBoton();


/* ----Impedir fechas no validas---- */
...
/* ----Imagenes permitidas---- */
imagen.addEventListener('change', () => { //Evento que se activa cuando el usuario añade el archivo
  const archivo = imagen.files[0];//Archivo subido por el usuario
  if (archivo && ['image/webp','image/png','image/jpeg'].includes(archivo.type)) return; //Si hay archivo subido y su tipo se permite no se muestra alerta,en caso contrario si
...
form.addEventListener('submit', (e) => {
  ...
  //Debe subirse unja imagen con el formato correcto
  const archivo = imagen.files[0];
  const fotoCorrecta = archivo && ['image/webp','image/png','image/jpeg'].includes(archivo.type);
  if (!fotoCorrecta) {
